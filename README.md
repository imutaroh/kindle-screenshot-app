...
> **注意**: このアプリは ad-hoc 署名（無料）でビルドしているため、アプリを再ビルドすると
> macOS が別アプリと判断し、画面収録・アクセシビリティの許可を再度求めることがある。
> その場合は「システム設定 > プライバシーとセキュリティ」の該当項目から古い PageSnap の
> エントリを削除（−ボタン）してから、改めて追加し直すこと。
